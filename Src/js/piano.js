  ...
  document.addEventListener("keydown", function (event) {
    ...
    switch (teclaPresionada) {
      case "a":
        ...
        cambiarImagen(
          teclasBlancas[0],
          "/Public/teclas/tecla_blanca_pulsada.svg"
        );
        break;
      case "s":
        reproducirAudio("audiore");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("keydown", function (event) {
    const teclaPresionada = event.key;
    const teclaBlanca = document.querySelector(
      `.tecla_blanca[data-key="${teclaPresionada}"]`
    );
    if (teclaBlanca) {
  ...
  document.addEventListener("keydown", function (event) {
    const teclaPresionada = event.key;
    const teclaNegra = document.querySelector(
      `.tecla_negra[data-key="${teclaPresionada}"]`
    );
    if (teclaNegra) {
